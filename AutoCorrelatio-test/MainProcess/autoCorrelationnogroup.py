...
if __name__ == '__main__':
    #################################################数据地址################################################################
    inputPath = r"/Volumes/Focus Work/TNSP_16d_kNDVI_MODIS_2002-2018_2014_2018/**/*.tif"
    ...
    for file in glob.glob(inputPath, recursive=True):
        ...
        if fileType == "tif":
            with rio.open(file) as ds:
                ...
                time = pd.to_datetime(datetime.datetime(year=int(year), month=int(month), day=1 if int(day) == 0 else 15))
                arr = xr.DataArray(data=singleImage, coords=[[time], lat, lon], dims=["time", "lat", "lon"]).to_dataset(
                    name=var)
    ...
    data.to_netcdf(outputPath + r"kNDVI.nc")
    data.close()
    #################################################整合数据################################################################

    ###############################################自相关系数################################################################
    with xr.open_dataset(outputPath + r"kNDVI.nc") as ds:
    ...
    for i in range(times.shape[0] - window_size):
        ...
        alpha, residual = linearRegression_cal("autoCorrelation",
                                               ...
                                               process_num=30,
                                               backup_path=r"/Volumes/Focus Work/Result" + "/")
    ...
    alpha_nc.to_netcdf(outputPath + r"autoCorrelation_alpha.nc")
    residual_nc.to_netcdf(outputPath + r"autoCorrelation_residual.nc")
    alpha_nc.close()
    residual_nc.close()
    ###############################################自相关系数################################################################

    ##################################################VarY##################################################################
    with xr.open_dataset(outputPath + r"autoCorrelation_alpha.nc")["alpha"] as alpha_nc:
        with xr.open_dataset(outputPath + r"autoCorrelation_residual.nc")["residual"] as residual_nc:
            lats = alpha_nc["lat"]
            lons = alpha_nc["lon"]
            times = alpha_nc["time"]
            ...
            var_y_nc.to_netcdf(outputPath + r"autoCorrelation_var_y.nc")
            var_y_nc.close()
    ##################################################VarY##################################################################
    ...
    nc_path = [
        outputPath + r"autoCorrelation_alpha.nc",
        outputPath + r"autoCorrelation_var_y.nc"
    ]
    for i, val in enumerate(val_names):
        ...
        nc.to_netcdf(outputPath + r"Statistics" + val + ".nc")
        nc.close()
    ##################################################结果统计###############################################################
