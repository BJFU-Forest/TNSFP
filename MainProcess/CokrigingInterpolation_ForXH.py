...
if __name__ == '__main__':
    ...
    # 1. 输入数据路径
    # 降尺度数据：
    downscaling_path = r"/Volumes/FluxGroup/GCM_P/Downscaling/Monthly" + "/"
    # 气象站站点信息：
    stationInfo = r"/Users/hx/Desktop/12th/Code/Downscaling/dataFile/Station/TNR_756.csv"
    ...
    # 插值时间分辨率
    temporal = "year"  # 仅支持"year"/"monthly"/"daily", 其他时间尺度需单独修改156行代码
    # 插值空间分辨率
    resolution = 0.005   # 分辨率过高时会出现圆斑
    ...
    for gcm in gcm_basename:
        for ssp in scenarios:
            for var in variables:
                ...
                if df.shape[0] == 0:
                    break
                df.to_csv(f"/Volumes/FluxGroup/GCM_P/Interpolation/{gcm}_{ssp}_{var}.csv")
                print("\n")
                # 协克里金插值
