...
if __name__ == '__main__':
    ...
    for j in range(group):
        ...
        for i, val in enumerate(val_names):
            ...
            for j in range(len(back_up_name)):
                print("\r%s_%s results are stored in NetCDF Dataset (xarray)" % ("stats_autoC", back_up_name[j]),
                      end="")
                nc[var + "_" + back_up_name[j]] = xr.DataArray(back_up_matrix[j], coords=[lats, lons],
                                                               dims=["lat", "lon"])
    ...
    integrated.to_netcdf(outputPath + r"Statistics_all.nc")
    integrated.close()
    ##################################################结果统计###############################################################
