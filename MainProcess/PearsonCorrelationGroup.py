...
if __name__ == '__main__':
    ...
    outputPath = r"/Volumes/home/HDD/ACDLM/" + "/"
    ...
    # #                     var, year, index = file_name.split("_")
    ...
    # #                     ###################################筛选植被区域######################################################
    # #                     month = int(index) // (2 * 16) + 1
    # #                     day = int(index) % (2 * 16)       #
    # #                     time = pd.to_datetime(
    # #                         datetime.datetime(year=int(year), month=int(month), day=1 if int(day) == 1 else 15))
    # #                     arr = xr.DataArray(
    ...
    # #     data.to_netcdf(outputPath + r"NPP_%d.nc" % i)
    # #     data.close()
    # # # ################################################整合数据################################################################
    #
    #
    ...
    #         ##########################################统计##############################################################
    #         print(">>>>> 统计 ", end="\n")
    #         avg, mk, sen = stats_cal("stats_" + val, matrix[69:, :, :], process_num=8,
    ...
    #     alpha = xr.open_dataset(outputPath + r"autoCorrelation_alpha_%d.nc" % i)["alpha"]
    #     alpha = alpha.groupby('time.year').mean(dim='time')
    #     alpha.to_netcdf(outputPath + r"alpha_year_%d.nc" % i)
    # ##################################################年尺度聚合###############################################################
    ...
    for var in variable:
        for i in range(group):
            ...
            #########################################统计##############################################################
            print(">>>>> 统计 ", end="\n")
            r, p = pearson_cal("stats_PearsonC", alphaM[46:, :, :], nppM[46:, :, :], process_num=8,
                               backup_path=r"/Volumes/home/HDD/backup/" + "/")
