...
class EEMD:
    ...
    **Ensemble Empirical Mode Decomposition**
    ...
    than simple Empirical Mode Decomposition (PyEMD). The robustness is
    ...
    trials : int (default: 100)
        Number of trials or PyEMD performance with added noise.
    ...
    ext_EMD : PyEMD (default: None)
        One can pass PyEMD object defined outside, which will be
        used to compute IMF decompositions in each trial. If none
        is passed then PyEMD with default options is used.
    parallel : bool (default: False)
        Flag whether to use multiprocessing in EEMD execution.
        Since each PyEMD(s+noise) is independent this should improve execution
        speed considerably.
    ...
    separate_trends : bool (default: False)
        Flag whether to isolate trends from each PyEMD decomposition into a separate component.
    ...
    def eemd(self, S: np.ndarray, T: Optional[np.ndarray] = None, max_imf: int = -1) -> np.ndarray:
        ...
        # For trial number of iterations perform PyEMD on a signal
    ...
    def _trial_update(self, trial) -> Tuple[np.ndarray, Optional[np.ndarray]]:
        """A single trial evaluation, i.e. PyEMD(signal + noise).
    ...
    def emd(self, S: np.ndarray, T: np.ndarray, max_imf: int = -1) -> np.ndarray:
        """Vanilla PyEMD method.

        Provides emd evaluation from provided PyEMD class.
        For reference please see :class:`PyEMD.PyEMD`.
        """
    ...
    def get_imfs_and_residue(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if self.E_IMF is None or self.residue is None:
            raise ValueError("No IMF found. Please, run PyEMD method or its variant first.")
...
if __name__ == "__main__":
    ...
    eemd = EEMD(trials=50, separate_trends=False)
