...
class EMD:
    ...
    **Empirical Mode Decomposition**
    ...
    >>> emd = PyEMD(extrema_detection='parabol')
    >>> IMFs = emd.emd(S)
    >>> IMFs.shape
    (1, 100)
    """
    ...
    def __init__(self, spline_kind: str = "cubic", nbsym: int = 2, **kwargs):
        ...
        MAX_ITERATION : int (default 1000)
            Maximum number of iterations per single sifting in PyEMD.
        ...
        total_power_thr : float (default 0.005)
            Threshold value on total power per PyEMD decomposition.
        range_thr : float (default 0.001)
            Threshold for amplitude range (after scaling) per PyEMD decomposition.
        ...
        Examples
        --------
        >>> emd = PyEMD(std_thr=0.01, range_thr=0.05)
    ...
    def find_extrema(self, T: np.ndarray, S: np.ndarray) -> FindExtremaOutput:
        ...
        ``extrema_detection`` variable, set on initiation of PyEMD.
    ...
    def end_condition(self, S: np.ndarray, IMF: np.ndarray) -> bool:
        """Tests for end condition of whole PyEMD. The procedure will stop if:
        ...
        * Metric L1 (mean absolute difference) is below *total_power_thr* threshold.
        ...
        ----------
        S : numpy array
            Original signal on which PyEMD was performed.
        IMF : numpy 2D array
