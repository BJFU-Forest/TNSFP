        ...
        if np.isnan(hisAc) or np.isnan(hisP) or np.isnan(hisET) or np.isnan(hisLUC) or np.isnan(futureP):
            ...
            try:
                if hisLUC in [1, 2, 3, 4, 5]:
                    suti = 1 if allP < 400 else 0
                    ac0 = 1 if hisAc > 0.429 else 0
                    etF = 0.0004 * futureP + 0.75
                    p_et_f = futureP - hisNPP * etF
                    # p_et_h = hisP - (0.1688 * hisP + 355.2602)
                    # p_et_f = futureP - (0.1688 * futureP + 355.2602)
                    # ach = 0.1880 + 1.4952 * 10e-5 * p_et_h + 6.1648 * 10e-7 * (p_et_h ** 2)
                    # acf = 0.1880 + 1.4952 * 10e-5 * p_et_f + 6.1648 * 10e-7 * (p_et_f ** 2)
                    risk = (1 if (hisP - futureP) >= 0 else 0) and ac0
                elif hisLUC == 6:
                    suti = 1 if (allP < 315) else 0
                    ac0 = 1 if hisAc > 0.489 else 0
                    # p_et_h = hisP - (0.1428 * hisP + 288.6405)
                    # p_et_f = futureP - (0.1428 * futureP + 288.6405)
                    # ach = 0.3874 - 0.0003 * p_et_h
                    # acf = 0.3874 - 0.0003 * p_et_f
                    risk = (1 if (hisP - futureP) >= 0 else 0) and ac0
                else:   # hisLUC==10
                    suti = 1 if (allP < 175) else 0
                    ac0 = 1 if hisAc > 0.437 else 0
                    # p_et_h = hisP - (0.2881 * hisP + 142.6629)
                    # p_et_f = futureP - (0.2881 * futureP + 142.6629)
                    # ach = 0.0003 + 5.7250 * 10e-8 * p_et_h - 1.9083 * 10e-9 * (p_et_h ** 2)
                    # acf = 0.0003 + 5.7250 * 10e-8 * p_et_f - 1.9083 * 10e-9 * (p_et_f ** 2)
                    risk = (1 if (hisP - futureP) >= 0 else 0) and ac0
                classify[i] = 1 if (suti and risk) else 2 if (suti and (not risk)) else 3 if ((not suti) and risk) else 4
            except:
